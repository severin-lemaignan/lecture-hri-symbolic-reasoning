...
\subtitle{Symbolic Reasoning for HRI}
\date{}
...
\def\colorstmt#1 #2 #3\relax{\texttt{{\color{hriSec2Dark}#1} {\color{hriSec3}#2} {\color{hriSec1Dark}#3}}}
\def\stmt#1{\expandafter\colorstmt#1\relax}
...
\begin{frame}{The Symbol Grounding Problem}
    ...
    \only<2>{
        ...
        pisse}'' into English ``\emph{It's raining cats and dogs}''.
...
\begin{frame}{Statement, beliefs}
    ...
    \stmt{teddybear type Toy}

    \pause

    \stmt{teddybear isOn table\_1}

    \pause

    \stmt{human\_1 scaredOf robot\_1} (in the human's model only!)
...
\begin{frame}{Statement, beliefs (2)}
    ...
    \texttt{give(robot\_1, human\_1, teddybear)}
    }
...
\begin{frame}{Ontologies}
    ...
    \begin{itemize}
        \item \textbf{T-box} statements: the \emph{conceptualisation} of the
            domain, for instance in terms of \emph{categories} (classes): 

            \begin{center}
                \bubblemark{class}\stmt{Dog rdfs:subClassOf Animal}
            \end{center}

        \item \textbf{A-box} statements: (T-box compliant) statements about
            \emph{individuals} (instances) in the ontology:

            \begin{center}
            \bubblemark{instance}\stmt{SPOT rdf:type Dog}
            \end{center}

    \end{itemize}

    \bubble<1>[0][0.5][3cm]{class}{{\tt Dog} is a class}
    \bubble<1>[0][0.5][3cm]{instance}{{\tt SPOT} is an instance}
...
\begin{frame}{Example}
    ...
    \begin{columns}
        ...
        \begin{tikzpicture}[
            ...
            instance/.style={draw=hriSec1Dark, font=\sf, node distance=0.5, rounded corners, align=center, inner sep=5pt, fill=none}]
        ...
        \node[fill=hriSec3CompDark!50, below right=of sthing] (location) {physical support} edge (sthing);
        \node[fill=hriSec3CompDark!50, below right=of artifact] (table) {table} edge (location) edge (artifact);
        \node[fill=hriSec3CompDark!50, below=of artifact] (book) {book} edge (artifact);
        \node[node distance=1, below right=of thing] (tthing) {temporal thing} edge (thing);
        \node[below right=of tthing] (evt) {event} edge[dashed] (tthing);
        \node[below right=of evt] (act) {action} edge (evt);
        \node[fill=hriSec1CompDark!50, right=2.5 of table] (color) {color} edge (thing);

        \node[rotate=90,draw=none,fill=none] at (-7.8,-2) {\bf\Large T-Box};

        \draw[dotted, thick] (-8,-3.8) -- +(16, 0);
            \node[instance, below=1.5 of color] (brown) {brown} edge[classof] (color);
        \node[rotate=90,draw=none,fill=none] at (-7.8,-5) {\bf\Large A-Box};
        ...
        \node[instance, right=of human, anchor=north west] (hpbook) {harry\_potter} edge[classof] (book);
        \node[instance, right=2 of robot] (ikea) {ikea\_table} edge[classof, bend right] (table);
        ...
        \path (moving.west) edge [property, out=180, in=-80, looseness=1.2] node[propname,below] {currentlyPerforms} (human.230);
        \path (hpbook.200) edge [property, out=-90, in=-90, looseness=2] node[propname,right] {looksAt} (human.300);
        \path (ikea.south) edge [property, out=-60, in=-60, looseness=2.5] node[propname, auto] {isOn} (hpbook.340);
        \path (brown) edge[property,out=-90, in=-90,looseness=1.5] node[propname,right,near start] {hasColor} (hpbook.south);

            \node[rotate=90,draw=none,fill=none,text width=2cm] at (-7.8,-7.5) {\bf\Large A-Box (on-line)};
        }
    ...
    \onslide<4>{
        \scriptsize
        The robot starts with some manually provided commonsense knowledge (encoded
        in T-Box + A-Box) and initial knowledge about the environment (in
        A-Box), and instantiate itself additional concepts and relations on-line, based
        on its perception.  }
...
\begin{frame}[fragile]{Example of first-order logic reasoning}
    ...
    \bubble<1>[90][0.5][5cm]{sparql}{Using SPARQL query language}
...
;
                ...
                right] (table);
                \node [instance, right=2 of book] (red) {brown} edge[property] node[propname] {hasColor} (book);
